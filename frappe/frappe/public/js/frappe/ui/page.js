 ...
 * @param {boolean} opts.custom_page
...
frappe.ui.Page = class Page {
	...
	setup_sidebar_toggle() {
		...
		if(this.custom_page){
			$("head").append(`<link type="text/css" rel="stylesheet" href="/assets/law/css/pages-custom.css">`)
			this.sidebar.html(
				`<ul class="custom-sidebar overlay-sidebar"></ul>`
			);
			let sidebar_data;
			frappe.call({
				method: "law.cases.page.pages_data.get_sidebar_data",
				callback: function(r) {
					// clean_emp_data = r.message
					sidebar_data = r.message
				},
				async: false
			});
			console.log(sidebar_data)
			this.$sidebar_list = this.sidebar.find("ul");
			sidebar_data.map(({title, page, icon})=>{
				this.get_sidebar_item(title, page, icon).appendTo(this.$sidebar_list);
			})
			this.$sidebar_list.on("click", "li", (e) => {		
				let $li = $(e.currentTarget);
				let sidebar_item = $li.find(".sidebar-text").attr("sidebar-value");
				// this.$sidebar_list.find("li").removeClass("selected-sidebar");
				// $li.addClass("selected-sidebar");
				// frappe.set_route(sidebar_item)
				location.href=sidebar_item
			})
			
		}
	}
	get_sidebar_item(title, item, icon) {
		let icon_html = icon ? frappe.utils.icon(icon, "md") : "";
		return $(`<li id="${item}" class="standard-sidebar-item-custom">
			<span>${icon_html}</span>
			<a class="sidebar-link" >
				<span class="sidebar-text" sidebar-value="${"/app/" + item}">${__(title)}</span>
			</a>
		</li>`);
	}
