...
MyPage = Class.extend({
	...
	make: function(){
		...
		let total_requests = function(){
			frappe.call({
				method: "law.cases.page.request_page.request_page.employee_data", //dotted path to server method
				callback: function(r) {
					...
					r.message["data"].forEach((requestData)=>{
									...
									<div class="card-body">
										<div class="row">
											<div class="col-10">
												...
												<div class="row pt-2">
													<div class="col-3">${requestData.claimant}</div>
													<div class="col-2">${requestData.name}</div>
													<div class="col-3">${requestData.starting_date}</div>
												...
												<div class="row">
												 	<div class="col-12">${requestData.legal_adaptation}</div>
												</div>
											</div>
						`
						if(isAllowed){
						...
						$(`#accept-${requestData.name}`).click(()=>{
							...
							frappe.call({
								method: "law.cases.page.request_page.request_page.update_doc", //dotted path to server method
