...
def vacation_data():
	...
	now()) as left_days from `tabVacation` inner join `tabEmployee` on	`tabEmployee`.name = `tabVacation`.employee where Date(now()) BETWEEN Date(`tabVacation`.start_date) AND Date(`tabVacation`.end_date)""", as_dict=True)
