...
MyPage = Class.extend({
	...
	make: function(){
		...
		let setup_functions = function(){
			context['vacations'].forEach((value) => {
				...
				$(`#accept-${value.name}`).on('click', ()=>{
					...
					frappe.call({
						method: "law.consumer.page.vacation_page.vacation_page.update_doc", //dotted path to server method
		...
		let total_requests = function(){
			frappe.call({
				method: "law.consumer.page.vacation_page.vacation_page.vacation_data", //dotted path to server method
				callback: function(r) {
